...
const startDate = new Date("2023-07-11T08:03:25Z");
const endDate = new Date("2023-12-06T08:45:27Z");

const aggregate = expenses
  .filter((expense) => {
    const expenseDate = new Date(expense.date);
    return expenseDate >= startDate && expenseDate <= endDate; // Date comparison
  })
  .reduce((acc, expense) => {
    if (!acc[expense.category]) {
      acc[expense.category] = 0; // Initialize category with 0
    }
    acc[expense.category] += expense.amount; // Sum the amount for each category
    return acc;
  }, {});

const finalData = Object.entries(aggregate).map(([key, value]) => ({
  [key]: value,
}));
console.log("Aggregate :", finalData);
console.log("Aggregate Length:", finalData.length);
